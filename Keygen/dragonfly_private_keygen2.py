...
#bind socket to port
# server_address = ('192.168.0.3', 65433)
server_address = ('192.168.0.3', 7000)
...
def handshake():
    ...
    while True:
        print("waiting for 192.168.0.21")
        sock.listen(1)
        connection, client_address = sock.accept()
        print(client_address[0])
        if (client_address[0]) != '192.168.0.22':
            connection.close()
            continue
        else:
            sock.close()
            with connection:
                ...
                while (content):
                    connection.sendall(priv_key_BER)
                    # print(len(priv_key_BER))
                    # print()
                    # print('sent', trying_encoding)
                    # print()
                    content = s.read(8192)
                    priv_key_BER = asn1_file.encode('DataKey', {'data': content})
                s.close()
